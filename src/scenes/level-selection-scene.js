  ...
  createCanvas() {
    ...
    this.levelButtonpos=[
      [
        ...
        [this.canvas.width/3+this.canvas.width/2.1-offsetCoordinateValue,this.canvas.height/6-offsetCoordinateValue],
        ...
        [this.canvas.width/3+this.canvas.width/2.1-offsetCoordinateValue,this.canvas.height/1.8-offsetCoordinateValue],
   ...
   document.getElementById(this.id).addEventListener("touchend",function(event){
    ...
    var touch = event.changedTouches[0];
        ...
        for (let s of gs.levels) {
            if (Math.sqrt((s.x+(imageSize/8))-x/1.3) <15 && Math.sqrt((s.y+(imageSize/8))-y)<12 ) {
                console.log(s.index);
              }
        }
